        ...
        case "SEND":
            try:
                event[key] = ones(6, dtype=float32) * deg2rad(
                    clip(float(app.input_field.get()), -60, 60)
                )
...
class App(ctk.CTk):
    ...
    def create_widgets(self) -> None:
        ...
        self.enable_button = ctk.CTkButton(
            self, text="ENABLE", command=lambda: button_callback({"ENABLE": None}, app=self)
        )
        ...
        self.disable_button = ctk.CTkButton(
            self, text="DISABLE", command=lambda: button_callback({"DISABLE": None}, app=self)
        )
        ...
        self.upload_button = ctk.CTkButton(
            self, text="UPLOAD", command=lambda: button_callback({"UPLOAD": None}, app=self)
        )
        ...
        self.stage_button = ctk.CTkButton(
            self, text="STAGE", command=lambda: button_callback({"STAGE": None}, app=self)
        )
        ...
        self.park_button = ctk.CTkButton(
            self, text="PARK", command=lambda: button_callback({"PARK": None}, app=self)
        )
        ...
        self.estop_button = ctk.CTkButton(
            self, text="ESTOP", command=lambda: button_callback({"ESTOP": None}, app=self)
        )
        ...
        self.reset_button = ctk.CTkButton(
            self, text="RESET", command=lambda: button_callback({"RESET": None}, app=self)
        )
